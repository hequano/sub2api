...
func (h *SettingHandler) UpdateSettings(c *gin.Context) {
	...
	// Cap (TryCap) 验证码参数验证
	if capEnabled {
		if strings.TrimSpace(req.CapApiEndpoint) == "" {
			response.BadRequest(c, "Cap API Endpoint is required when enabled")
			return
		}
		normalizedEndpoint, err := service.NormalizeCapAPIEndpoint(req.CapApiEndpoint)
		if err != nil {
			response.BadRequest(c, err.Error())
			return
		}
		req.CapApiEndpoint = normalizedEndpoint
		...
		if err := service.ValidateCapSiteKey(req.CapSiteKey); err != nil {
			response.BadRequest(c, err.Error())
			return
		}
		if req.CapSecretKey == "" {
