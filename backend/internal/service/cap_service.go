...
// NormalizeCapAPIEndpoint validates and canonicalizes a Cap server base URL.
func NormalizeCapAPIEndpoint(raw string) (string, error) {
	endpoint := strings.TrimSpace(raw)
	if endpoint == "" {
		return "", fmt.Errorf("cap API endpoint is required")
	}
	...
	if err != nil {
		return "", fmt.Errorf("invalid cap API endpoint: %w", err)
	}
	if (parsed.Scheme != "http" && parsed.Scheme != "https") || parsed.Host == "" {
		return "", fmt.Errorf("cap API endpoint must be an HTTP(S) URL")
	}
	if parsed.User != nil || parsed.RawQuery != "" || parsed.Fragment != "" {
		return "", fmt.Errorf("cap API endpoint must not contain credentials, a query, or a fragment")
	}
...
// ValidateCapSiteKey ensures the key can be appended as one URL path segment.
func ValidateCapSiteKey(siteKey string) error {
	siteKey = strings.TrimSpace(siteKey)
	if siteKey == "" {
		return fmt.Errorf("cap site key is required")
	}
	if siteKey == "." || siteKey == ".." || strings.ContainsAny(siteKey, `/\?#:`) {
		return fmt.Errorf("cap site key must be a single URL path segment")
	}
	return nil
}
...
func (s *CapService) VerifyTokenWithConfig(ctx context.Context, config CapConfig, token, remoteIP string) error {
	...
	if _, err := NormalizeCapAPIEndpoint(apiEndpoint); err != nil {
		return ErrCapNotConfigured
	}
	if err := ValidateCapSiteKey(siteKey); err != nil {
		return ErrCapNotConfigured
	}
...
func (s *CapService) ValidateCredentials(ctx context.Context, apiEndpoint, siteKey, secretKey string) error {
	...
	if _, err := NormalizeCapAPIEndpoint(apiEndpoint); err != nil {
		return fmt.Errorf("%w: %v", ErrCapNotConfigured, err)
	}
	if err := ValidateCapSiteKey(siteKey); err != nil {
		return fmt.Errorf("%w: %v", ErrCapNotConfigured, err)
	}

	probeToken := siteKey + ":sub2api:credential-validation"
	result, err := s.verifier.VerifyToken(ctx, apiEndpoint, siteKey, secretKey, probeToken, "")
	if err != nil {
		return fmt.Errorf("validate cap credentials: %w", err)
	}
	if result == nil {
		return fmt.Errorf("validate cap credentials: empty response")
	}
	if result.Success {
		return nil
	}
	...
	if message == "token not found" || message == "token expired" {
		return nil
	}
	if message == "invalid site key or secret" {
	...
	return fmt.Errorf("validate cap credentials: %s", strings.TrimSpace(result.Error))
}
