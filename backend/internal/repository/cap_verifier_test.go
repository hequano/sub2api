...
func TestCapVerifier_ReturnsStructuredErrorResponse(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusForbidden)
		_ = json.NewEncoder(w).Encode(map[string]any{
			"success": false,
			"error":   "Invalid site key or secret",
		})
	}))
	defer server.Close()

	verifier := &capVerifier{httpClient: server.Client()}
	resp, err := verifier.VerifyToken(context.Background(), server.URL, "test-site", "bad-secret", "test-site:a:b", "")
	require.NoError(t, err)
	require.False(t, resp.Success)
	require.Equal(t, http.StatusForbidden, resp.StatusCode)
	require.Equal(t, "Invalid site key or secret", resp.Error)
}
