export default {
  promptAudit: {
    title: '提示词审计',
    description: '独立记录适用请求及完整提示词，供管理员查询复核，无需配置 AI 节点；配置节点后可选启用 AI 风险识别或同步阻止。',
    ...
    mode: { off: '已关闭', async_audit: '请求记录 / 可选 AI 审计', blocking: '同步 AI 审计并阻止' },
    status: { disabled: '未启用', running: '运行中', degraded: '降级', error: '错误', healthy: '健康', failed: '失败', stale: '心跳过期' },
    decisions: { pass: '通过', flag: '标记', critical: '严重', recorded: '已记录' },
    riskLevels: { low: '低', medium: '中', high: '高', critical: '严重', unassessed: '未评估' },
    scanners: {
      violent: '暴力',
    ...
    pool: {
      title: '可选 AI 审计节点', description: '不配置节点时仅保存请求记录；配置后才会调用 AI 做风险识别。探测由服务端真实网络环境发起。',
      add: '新增节点', edit: '编辑节点', empty: '尚未配置 AI 节点；当前将仅保存请求记录，不执行 AI 审计。', node: '节点', model: '模型', limits: '超时 / 单片上限', credential: '凭据与探测',
    ...
    policy: {
      title: '记录范围与 AI 策略', description: '适用分组决定保存哪些请求；风险分类、Worker 和队列仅在配置 AI 节点后使用。', scope: '适用范围', allGroups: '全部分组', selectedGroups: '指定分组',
    ...
    saveBar: { enabled: '启用请求记录', blocking: '同步 AI 阻止', blockingLatestTurnOnly: '仅审最新输入和上一轮输出', storePass: '保存全部记录', retentionDays: '保留天数', dirty: '有未保存的更改', synced: '配置已同步' },
    blockingConfirm: {
      title: '开启同步阻止？',
    ...
    events: {
      title: '请求记录', description: '按身份、入口、Hash 和时间查询记录，详情中可查看完整提示词；未调用 AI 的记录会显示为“未评估”。', decision: '状态', risk: '风险等级', endpoint: '入口', groupId: '分组 ID', userId: '用户 ID', apiKeyId: 'API Key ID', keyword: '关键词',
      ...
      recordOnly: '仅记录（未调用 AI）', recordOnlyHint: '该请求仅做留档，没有调用 AI 模型，因此没有风险判定结果。',
      riskSummaries: '风险摘要',
      evidence: '脱敏证据',
      score: '分数',
    ...
    errors: {
      ...
      prompt_guard_requires_audit_enabled: '开启同步阻止前必须先启用请求记录。', prompt_audit_invalid_endpoint: 'AI 审计节点配置无效。', prompt_audit_endpoint_required: '开启同步 AI 阻止前至少需要一个启用节点；仅保存请求记录不需要节点。', prompt_audit_groups_required: '指定分组模式至少需要选择一个分组。', prompt_audit_scanners_required: '至少需要启用一个风险分类。', prompt_audit_invalid_event_retention: '事件保留天数需在 1 到 3650 天之间。',
    },
  },
}
